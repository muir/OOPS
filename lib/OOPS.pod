...
On rare occasion PostgresSQL will fail with an
...
cannot be queried or they are a mapping from a relational database
into Perl data that does not allow arbitrary data to be stored.

L<http://poop.sourceforge.net/> has an overview of the options.
...
OOPS may be used, modified and redistributed on the same terms
as Perl itself.
